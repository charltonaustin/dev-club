...
RSpec.describe 'MainScreen' do
  ...
  context 'when condition' do
    it 'succeeds' do
      ...
      expect($stdout).to have_received(:puts).with("4. Delete a task").exactly(5)
      expect($stdout).to have_received(:puts).with("5. Edit a task").exactly(5)
      expect($stdout).to have_received(:puts).with("6. View task history").exactly(5)
      expect($stdout).to have_received(:puts).with("7. Revert to previous version").exactly(5)
